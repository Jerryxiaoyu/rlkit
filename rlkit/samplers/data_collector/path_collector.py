...
class GoalConditionedDictPathCollector(PathCollector):
    def __init__(
            self,
            env,
            policy,
    ...
    def collect_mp_new_paths(self):
        ...
        pool = mp.Pool()
        multi_res = [pool.apply_async(job3, (i,)) for i in range(10)]  # 利用迭代器
        # 从迭代器中取出
    ...
    def get_snapshot(self, is_eval=False):
        snap = dict(
            env=self._env,
            #policy=self._policy,
            observation_key=self._observation_key,
